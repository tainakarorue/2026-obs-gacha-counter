...
function buildTweetText() {
  ...
  if (state.creditPerPull > 0) {
    const totalCredit = state.totalCount * state.creditPerPull;
    text += `💎 消費石数: ${totalCredit.toLocaleString()}個\n`;
  }
...
function init() {
  ...
  // 広告トグル（セッション中のみ。毎回訪問時は表示状態でスタート）
  ...
  adToggle.addEventListener('click', () => {
    const isHidden = adArea.classList.toggle('hidden');
    adToggle.textContent = isHidden ? '広告を表示 ▼' : '広告を隠す ▲';
  });
}
